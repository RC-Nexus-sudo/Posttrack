...
App.templates = {
 ...
 renderView: function(id, title) {
  ...
  headerAction = `
  <button onclick="App.modules.entrants.openForm()" class="bg-blue-600 text-white px-5 
py-2.5 rounded-2xl text-sm font-bold shadow-lg shadow-blue-100 hover:bg-blue-700 transition flex 
items-center gap-2">
  ...
  <p class="text-slate-400 text-xs mb-6">Redirection sécurisée vers le registre des 
habilitations...</p>
  <a href="admin.html" class="inline-block bg-blue-600 text-white px-6 py-3 rounded-xl 
font-bold text-xs uppercase tracking-widest">Ouvrir</a>
  </div>`;
  } 
  else {
  body = `<div class="p-10 bg-slate-50 border border-dashed rounded-[2.5rem] text-slate-400 
text-center italic">Module ${title} en attente.</div>`;
  }
  return `
 ...
 // 3. FORMULAIRE DE SAISIE (MODAL) (MODIFIÉ)
 entryForm: function() {
  return `
  <div class="p-10">
  <header class="mb-8">
  <h3 class="text-2xl font-black text-slate-800 tracking-tight">Nouvel Enregistrement</h3>
  <p class="text-slate-400 text-sm italic font-medium">Saisie rigoureuse du courrier 
entrant</p>
  </header>
  ...
  <select id="mail-type" class="w-full p-4 bg-slate-50 border-none rounded-2xl 
outline-none focus:ring-2 focus:ring-blue-600 font-medium text-sm">
  ...
  <input type="text" id="mail-sender" placeholder="Nom, Institution ou Entreprise..." 
class="w-full p-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-blue-600 outline-none 
transition font-medium text-sm">
  ...
  <select id="mail-dest-service" class="w-full p-4 bg-slate-50 border-none rounded-2xl 
outline-none focus:ring-2 focus:ring-blue-600 font-medium text-sm">
  <option value="">Chargement des services...</option>
  </select>
  </div>
  <!-- 5. Description/Objet (Le champ existant est réutilisé ici) -->
  <div class="md:col-span-2">
  <label class="block text-[10px] font-black uppercase text-slate-500 mb-2 ml-1 tracking-widest">5. Description de l'objet</label>
  <textarea id="mail-subject" rows="3" placeholder="Contenu sommaire ou référence du 
dossier..." class="w-full p-4 bg-slate-50 border-none rounded-2xl outline-none focus:ring-2 
focus:ring-blue-600 font-medium text-sm"></textarea>
  </div>
  
  <!-- 6. AJOUT DU CHAMP FICHIER -->
  <div class="md:col-span-2">
      <label for="mail-file" class="block text-[10px] font-black uppercase text-slate-500 mb-2 ml-1 tracking-widest">6. Fichier joint (JPG/PDF)</label>
      <input type="file" id="mail-file" accept=".jpg, .jpeg, .pdf"
  ...
  <button onclick="document.getElementById('modal-overlay').classList.replace('flex', 
'hidden')" class="px-8 py-4 text-slate-400 font-bold hover:text-slate-600 transition text-xs uppercase 
tracking-widest">
  Annuler
  </button>
  </div>
